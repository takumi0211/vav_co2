...
- **`weather_data/`** – 31 CSVs (`outdoor_temp_202507XX.csv`) fetched from the JMA 10-min archive. Regenerate with `python -m simulator_humid.data_tools.scrape_weather`. `simulator_humid.simulation.OUTDOOR_DATA_PATH` defaults to `weather_data/outdoor_temp_20250729.csv`; update the constant or override upstream before running alternative weather.
