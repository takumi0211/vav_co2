...
class TrainingConfig:
    ...
    exploration_noise: float = 0.1  # 環境ステップでの追加アクションノイズ標準偏差
...
class SACActor(nn.Module):
    ...
    def sample(self, obs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return squashed, log_prob, deterministic
        ...
        if should_sample:
            with torch.no_grad():
                if self.update_normalizer:
                    ...
                    squashed = self.policy.deterministic(obs_tensor)
                if self.update_normalizer and self.exploration_noise > 0.0:
                    noise = torch.randn_like(squashed) * float(self.exploration_noise)
                    if self.noise_clip > 0.0:
                        clip_val = float(abs(self.noise_clip))
                        noise = torch.clamp(noise, -clip_val, clip_val)
                    squashed = torch.clamp(squashed + noise, -0.999, 0.999)
...
def export_teacher_dataset(
    *,
    ...
    """リプレイバッファからサンプルを抽出し、SACクリティックの教師データセットを保存する。"""
    ...
    alpha_value = float(log_alpha.detach().exp().cpu().item())
    ...
    for start in range(0, sample_size, chunk_size):
        ...
        with torch.no_grad():
            ...
            if config.teacher_use_target_q:
                next_action, next_log_prob, _ = actor.sample(next_obs_norm)
                next_action = torch.clamp(next_action, -0.999, 0.999)
                target_q1, target_q2 = target_critic(next_obs_norm, next_action)
                alpha_tensor = torch.as_tensor(alpha_value, device=next_log_prob.device, dtype=next_log_prob.dtype)
                target_min = torch.min(target_q1, target_q2) - alpha_tensor * next_log_prob
    ...
    print(f"[SAC] Exported critic teacher dataset: {file_path} ({sample_size} samples)")
    return file_path
...
def train(config: TrainingConfig) -> None:
    ...
    log_path = config.output_dir / "training_log_sac_no_randomize_occupancy.csv"
    ...
    if config.randomize_occupancy:
        try:
            ...
            print(f"[SAC] Occupancy profiles unavailable ({exc}); falling back to default schedules.")
            occupancy_profiles = None
    ...
    initial_alpha = max(float(config.initial_alpha), 1e-5)
    log_alpha = torch.tensor(math.log(initial_alpha), device=device, requires_grad=True)
    ...
    target_entropy_tensor = torch.tensor(target_entropy, device=device, dtype=torch.float32)

    def sac_gradient_step(allow_actor_gradients: bool) -> tuple[float, float | None, float | None, float]:
        """SACの1ステップ勾配更新を実行し、損失を返す。"""
        nonlocal total_updates, log_alpha
        ...
        with torch.no_grad():
            ...
            alpha_detached = log_alpha.detach().exp()
            target_min = torch.min(target_q1, target_q2) - alpha_detached * next_log_prob
            target_value = batch["rewards"] + (1.0 - batch["dones"]) * config.gamma * target_min
        ...
        if allow_actor_update:
            ...
            temperature_loss = -(log_alpha * (log_prob + target_entropy_tensor).detach()).mean()
        ...
        alpha_snapshot = float(log_alpha.detach().exp().cpu().item())
        return q_loss_value, actor_loss_value, temperature_loss_value, alpha_snapshot
    ...
    for episode in range(1, config.episodes + 1):
        ...
        if len(replay_buffer) >= config.batch_size and gradient_target > 0:
            for _ in range(gradient_target):
                q_loss, actor_loss_val, temp_loss, alpha_snapshot = sac_gradient_step(
                    allow_actor_gradients=True,
                )
                q_losses.append(q_loss)
                alpha_values.append(alpha_snapshot)
                if actor_loss_val is not None:
                    actor_losses.append(actor_loss_val)
                if temp_loss is not None:
                    temperature_losses.append(temp_loss)
        ...
        if config.critic_extra_epochs > 0 and len(replay_buffer) >= config.batch_size:
            ...
            for _ in range(config.critic_extra_epochs):
                for _ in range(batches_per_epoch):
                    q_loss, _, _, alpha_snapshot = sac_gradient_step(
        ...
        current_alpha = float(log_alpha.detach().exp().cpu().item())
    ...
    model_path = config.output_dir / "sac_policy_final_no_randomize_occupancy.pt"
    torch.save(
        {
